...
@implementation NPIntroductionView
@synthesize delegate;
...
- (id)initWithFrame:(CGRect)frame headerText:(NSString *)headerText panels:(NSArray *)panels
{
    ...
    if (self) {
        ...
        [self initializeClassVariables];
        Panels = [panels copy];
        LanguageDirection = NPLanguageDirectionLeftToRight;
        [self buildUIWithFrame:frame headerViewVisible:YES];
...
-(void)buildContentScrollViewWithFrame:(CGRect)frame{
    ...
    if (Panels) {
        if (Panels.count > 0) {
            ...
            if (LanguageDirection == NPLanguageDirectionLeftToRight) {
                [self buildContentScrollViewLeftToRight];
            }
            else if (LanguageDirection == NPLanguageDirectionRightToLeft) {
...
-(UIView *)PanelViewForPanel:(NPIntroductionPanel *)panel atXIndex:(CGFloat*)xIndex{
...
-(void)buildFooterView{
    ...
    //Build Skip Button
    if (LanguageDirection == NPLanguageDirectionRightToLeft) {
...
-(void)skipIntroduction{
    if ([(id)delegate respondsToSelector:@selector(introductionDidFinishWithType:)]) {
        [delegate introductionDidFinishWithType:NPFinishTypeSkipButton];
    }
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (LanguageDirection == NPLanguageDirectionLeftToRight) {
        ...
        if (self.CurrentPanelIndex == (panelViews.count)) {
            if ([(id)delegate respondsToSelector:@selector(introductionDidFinishWithType:)]) {
                [delegate introductionDidFinishWithType:NPFinishTypeSwipeOut];
            }
        }
        else {
            //Update Page Control
